...
if (($inputRecords = F_Input::getRaw("id", null))) {
  $inputRecords = explode(",", $inputRecords);
  foreach ($inputRecords as $inputRecordId) {
    $loaded[] = H_FoodRecord::load($inputRecordId);
  }
}
...
foreach ($input->input as $inputList) {
  foreach ($inputList as $inputRecord) {
    if ($inputRecord->foodid < 1) {
      continue;
    }
    $rec = H_FoodRecord::create();
    $rec->userid = $user->id;
    $rec->foodid = $inputRecord->foodid;
    $rec->amount = $inputRecord->amount;
    $loaded[] = $rec;
  }
}

if (($inputMeal = F_Input::getRaw("group", null))) {
  $inputDay = F_Input::getInteger("day", time());
  $inputDay = strtotime(date("Y-m-d", $inputDay) . " 00:00:00");
  $mealRecords = H_FoodRecord::query(
    "userid=" . $user->id . " AND time>=" . $inputDay . " AND time<" .
    ($inputDay + F_UtilsTime::A_DAY) . " AND `group`=" . $db->quote($inputMeal)
  );
...
if (($inputRecords = F_Input::getRaw("clone", null))) {
  $inputRecords = explode(",", $inputRecords);
  foreach ($inputRecords as $inputRecordId) {
    $copy = true;
    $loaded[] = H_FoodRecord::load($inputRecordId);
...
<table class='table table-noborders'>
  <input type="hidden" name="action" value="diet.edit.record" />
  <input type="hidden" name="if_copy" value="<?php echo $copy ? 1 : 0; ?>" />
  ...
  <tr>
      <td colspan='2' style='border-top-style: hidden;'>
        <table id="food-list-table" class="table table-striped">
  ...
  <tr>
      ...
      <td style='border-top-style: hidden;'>
        <input class="input" type="text" 
               id="group-field" 
               placeholder="group (e.g. breakfast, lunch, ...)" 
               name="if_group" value="<?php echo htmlspecialchars($input_group); ?>" />
...
<div class="form-actions">
  ...
  <?php if ($someLoaded) : ?>
  <button type="submit" name="action-copy" class="btn btn-primary">Save as copy</button>
  <?php endif; ?>
...
var today = "<?php echo date("d-m-Y", time()); ?>";
...
function removeFood(selectedid) {
  // remove from list
  if (selectedFoods[selectedid].recordid > 0) {
    selectedFoods[selectedid].status = 0;  
  }
  else {
    delete selectedFoods[selectedid];
  }
